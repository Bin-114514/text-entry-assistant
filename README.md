...
`v0.1.0-preview.1`。这是预览版：核心状态机、Unicode `SendInput` 后端、目标窗口检查、全局快捷键和 WPF 界面已经实现，兼容性仍以[记录](docs/compatibility.md)为准。
...
3. `Ctrl+Alt+P` 暂停或继续，继续前会重新检查目标；`Ctrl+Alt+Esc` 紧急停止。
4. 快捷键可在窗口中修改。若操作系统报告组合键已被占用，换用其他组合后重新应用。
